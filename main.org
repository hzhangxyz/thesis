...
#+LATEX: {\blue{WC: 这里应该加入一个小节，讲 MPS 的能力。定理：一维有 gap 短程相互作用系统基态可以被 MPS 表示。}}
#+LATEX: {\blue{ZH: 我在下面一节写了}}

*** {{{AL}}} 与 {{{MPS}}}

#+LATEX: {\blue{WC: 感觉纠缠熵面积定律应该放在 PEPS 部分}}
#+LATEX: {\blue{ZH: 可是MPS确实在1维时保证了面积定律啊，如果这里不写，到PEPS再写会很奇怪。}}
...
#+LATEX {\blue{WC: 感觉这部分应该放在 PEPS 部分}}
#+LATEX {\blue{ZH: 可是MPS里我们已经开始使用图形标记了}}
