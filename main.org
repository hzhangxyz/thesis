...
#+LATEX: {\color{blue}结果显示，在我们的参数范围中，系统会表现出反铁磁序和电荷密度波序。}
...
#+LATEX: {\color{blue}ZH: 原来的 ``当空穴掺杂较小时，系统表现出反铁磁序和电荷密度波序'' 和后文联系起来可能有歧义。这里的小dop是只我们的参数范围本身小，后文的小指范围内 $\delta<0.05$ 的小，故修改为现在版本。}
...
#+LATEX: {\color{blue}ZH: 第一句话引用的都是经典的多体问题，所以应该是上上世纪而不是上世纪。}
...
为了表达上的简便，我们直接称呼 $T$ 为张量， 即使严格的说张量应是其所表示的多线性映射 $\hat T$ 。
...
&= \sum_{i=0}^{N-1} \frac{1}{6} (\mathbf{S}^i \cdot \mathbf{S}^{i+1})^2 + \frac{1}{2} (\mathbf{S}^i \cdot \mathbf{S}^{i+1})^2 + \frac{1}{3}  ) \right) ,
\end{aligned}
\end{equation}
...
为了构造这样的基态，如图 [[ref:fig:aklt]] (a) 所示， 我可以在每两个格点之间构造一个 自旋 $\frac{1}{2}$ 的单重态，每个格点左右两侧各一个 自旋 $\frac{1}{2}$ 的粒子，
其可以表示为 自旋 $0$ 和 自旋 $1$ 的直和，我们使用 $P^1$ 将其投影到 自旋 $1$ 空间，从而得到AKLT的解空间。
...
(c) 单重态和两个自旋 $\frac{1}{2}$ 粒子的 自旋 $2$ 分量，为0；
(d) 两个自旋 $0$ 的自旋 $2$ 分量，为0 。
}
\label{fig:aklt}
\end{figure}
#+end_export

#+LATEX: {\color{blue} WC: 这里应该加入一个小节，讲 MPS 的能力。定理：一维有 gap 短程相互作用系统基态可以被 MPS 表示。}
#+LATEX: {\color{blue} ZH: 我在下面一节写了}

*** {{{AL}}} 与 {{{MPS}}}

#+LATEX: {\color{blue} WC: 感觉纠缠熵面积定律应该放在 PEPS 部分}
#+LATEX: {\color{blue} ZH: 可是MPS确实在1维时保证了面积定律啊，如果这里不写，到PEPS再写会很奇怪。}
...
#+LATEX: {\color{blue} WC: 感觉这部分应该放在 PEPS 部分}
#+LATEX: {\color{blue} ZH: 可是MPS里我们已经开始使用图形标记了}
...
#+LATEX: {\color{blue} WC: 可以将纠缠熵的面积定律章节移到这里}

*** {{{PEPS}}}的表示

#+LATEX: {\color{blue} WC: 可以将图形表示部分移到这里}
...
当系统并没有自发破缺对称性时，我们可以将 {{{Ansatz}}} 子空间限定为满足这个对称性的子空间，这一方面给出了先验的知识，是的拟设函数更容易找到系统真正的基态，另一方面，也减少了计算的代价。
而张量网络可以轻松地表示系统的多种对称性，这便是所谓的对称性张量网络态[[cite:Singh2010,Singh2011,Singh2012]]，其由对称性张量组成。

*** 对称性张量的定义

对称性变换可以使用一个紧致群 $G$ 来描述，
...
为此，多种适用于费米子的 {{{PEPS}}} 方法被提出 [[cite:Corboz2010,Corboz2011,Gu2010,Gu2013,Kraus2010,Dong2019]] ，而本节中，我们将介绍董少钧的费米子方案 [[cite:Dong2019]] 。

*** 费米子张量的定义

#+LATEX: {\color{blue} WC: 这没写费米子张量具体的数学表达式是啥。}
#+LATEX: {\color{blue} ZH: 我有提到公式 \eqref{eq:费米子的投影算符} }
...
由于费米子算符的引入，费米子张量的各个操作于寻常的张量不同，我们一个一个地进行了推导，并最终得到了一套费米子张量的运算法则。
...
由于费米子张量将费米子的细节完全掩盖在自身和各种张量运算中，各种针对 {{{PEPS}}} 的算法可以直接应用与费米子张量构成的{{{PEPS}}}中，
...
此外，我们成功将{{{TNSP}}}套件移植到国产超级计算机神威 \cdot 太湖之光上，并通过多层次有针对性的优化，提升软件性能 6 倍，同时在 40000 进程下达到了 90.74% 的并行效率。
