...
满足公式 [[eqref:eq:对称性张量]] 的张量被称作对称性张量，图 [[ref:fig:对称性张量和对称性张量网络]] (a)使用图形标记法展示了此关系。
...
其中 $s$ 标记了不同的不可约表示， $\alpha_s$ 简并空间的指标，即 $\alpha_s = 0,1,\cdots d_s-1$ ，而 $m_s$ 是这个不可约表示空间中的指标。
