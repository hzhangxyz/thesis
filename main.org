...
#+begin_src emacs-lisp :exports none :results none
  ...
  (org-link-set-parameters "cite"
                           ...
                           (cond
                             ((eq backend 'latex) (format "\\cite{%s}" path))
                             (t (format "[[cite:%s]]" path)))))
  ...
  (org-link-set-parameters "eqref"
                           :export (lambda (path desc backend)
                           (cond
                             ((eq backend 'latex) (format "\\eqref{%s}" path))
                             (t (format "[[eqref:%s]]" path)))))
...
* 基于自然梯度法的有限温算法 [fn:共同创作声明] <<sec:有限温>>
[fn:共同创作声明] 本章节工作由本人与张盟共同完成，我们共同参与方法的设计，张盟负责对自旋系统的模拟计算，本人负责对费米子系统的模拟计算。
