...
为了深入探究六角晶格中Hubbard模型掺杂后的性质，我们在一个较宽的参数范围内通过第 [[ref:sec:张量和张量网络]] 章中介绍的费米子投影纠缠对状态研究了掺空穴的六角晶格Hubbard模型。
...
\left\{
\begin{aligned}
\Delta_s^a(i) &= \frac{1}{\sqrt{2}}(c_{i\uparrow}c_{j\downarrow} - c_{i\downarrow}c_{j\uparrow}) \tag{1} ,\\
\Delta_t^a(i) &= \frac{1}{\sqrt{2}}(c_{i\uparrow}c_{j\downarrow} + c_{i\downarrow}c_{j\uparrow}) \tag{2} ,\\
\end{aligned}
\right.
\end{equation}
...
\caption{\label{fig:cooper-pairs}Cooper对的对称性}
\note{注：
库珀对配对对称性示意图：(a) 对于$s$波和$f$波配对，三个库珀对具有相同的相位；(b) 对于$p$波和$d$波配对，三个库珀对$\Delta_1$、$\Delta_2$和$\Delta_3$分别具有相同的、${\rm exp(\pm 2/3 \pi i)}$和${\rm exp(\pm 4/3 \pi i)}$相位。
}
\end{figure}
#+end_export
