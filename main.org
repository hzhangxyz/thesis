...
一维情况下，局部 {{{Hamiltonian}}} 构成的有能隙系统的基态是满足 {{{AL}}} 的 [[cite:Eisert2010]]，所以一个固定 $D$ 的 {{{MPS}}} 可以很好地表示此类系统的基态。
而对于无能隙的系统，有 $S \propto \alpha \ln L$ [[cite:Eisert2010]] ，即纠缠熵随着系统的尺寸对数上升，而 $S \propto \ln D$ ，所以应取 $D \propto L ^\alpha$ ，
