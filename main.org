...
为了构造这样的基态，我可以在每个两个格点之间构造一个 自旋 $\frac{1}{2}$ 的单重态，每个格点左右两侧各一个 自旋 $\frac{1}{2}$ 的粒子，
其可以表示为 自旋 $0$ 和 自旋 $1$ 的直和，我们使用 $P^1$ 将其投影到 自旋 $1$ 空间，从而得到AKLT的解空间。
注意到 $P_2 P^1 |\text{单重态}\rangle = P_2 |\text{单重态}\rangle - P_2 P^0 |\text{单重态}\rangle = 0 - 0 = 0$ ，所以这确实是AKLT模型的基态，
