...
尽管TNS方法在许多应用中表现优异，但在优化二维系统的基态和处理复杂相互作用时仍面临挑战。
此外，TNS方法也被推广到费米子系统。
各种费米子张量网络态（f-TNS）方法相继被开发。
然而，由于f-TNS方法的复杂性，其实现面临巨大的挑战，这种困难因缺乏支持f-TNS方法的基础软件包而进一步加剧。
为了解决张量网络，特别是费米子张量网络方法带来的挑战，本论文开展了以下工作：

一、为简化费米子张量网络及对称张量网络的编程实现，我们提出了一种新方案，通过直接定义张量的运算规则，成功实现了费米子的反对易性质。
这使得在网络层次可以忽略费米子张量网络的复杂细节，简化了对对称性和费米子性质的处理，显著降低了编程的复杂性。
我们此前开发的TNS 方法软件包，TNSPackage，仅支持自旋和玻色子模型。
在最新版本的 TNSPackage 中，我们增加了对对称性张量和费米子张量的支持。
我们发展了一个统一的接口，可以一致性的处理玻色子、费米子和各种对称类型的张量，保持了软件包的用户友好性和多功能性。
这大大简化了基于 f-TNS 的程序开发过程。新的 TNSP 框架包含两个主要组成部分：一个底层张量库 TAT，支持复杂的张量运算；以及基于 TAT 构建的高级接口包 tetragono。
tetragono 专为简化在方形晶格上开发复杂物理模型而设计。 TNSPackage 框架使研究人员能够更加轻松地实现广泛的物理模型，而无需深入关注底层实现细节。

二、我们利用费米子投影纠缠对态（fPEPS）方法研究了在六角格子上掺杂空穴的哈伯德模型。
研究发现，在这个模型中库珀对存在准长程有序，其关联函数呈现幂律衰减，衰减指数 $K>1$。
我们进一步分析了超导性与其他竞争相之间的关系，包括反铁磁（AFM）序和电荷密度波（CDW）序。
结果显示，当空穴掺杂 $\delta$ 较小且库仑参数 $U$ 较大时，系统中存在畴壁结构。
然而，随着空穴掺杂 $\delta$ 的增加或 $U$ 的减少，这些畴壁结构会消失。
当空穴掺杂较小时，系统表现出反铁磁有序，但在 $\delta > 0.05$ 时该有序减弱。
相反地，随着掺杂水平的增加，电荷密度波有序逐渐减弱。
值得注意的是，即使在较高的掺杂水平下，仍然可以观察到显著的电荷密度波有序。
这些结果表明，随着掺杂水平 $\delta$ 的增加，反铁磁有序逐渐受到抑制，而电荷密度波有序则逐渐占据主导地位。

三、我们提出了一种可扩展的二维量子多体系统有限温度张量网络算法。
该算法使用（f-）PEPS来表示量子热态的矢量化，并通过随机重构方法（Stochastic Reconfiguration, SR）逐步将量子态从无限温度冷却。
为验证算法的有效性，我们对二维反铁磁海森堡模型、J1-J2模型和费米–哈伯德模型进行了测试。
物理性质如内能量、比热和磁化率等的计算结果与随机级数展开（SSE）、严格对角化和行列式量子蒙特卡洛（DQMC）方法的结果进行了对比，显示出高度一致性，验证了该算法的准确性和可靠性。
#+begin_export latex
\end{abstract}
#+end_export
...
This thesis presents the development and applications of tensor network states and their algorithms.
It primarily includes:
a scheme applying tensor network states to symmetry systems and fermionic systems,
where we implement the anti-commutation property of fermions by directly modifying tensor operation rules,
which allows us to ignore detailed fermionic properties at the network level,
thereby simplifying the algorithm design without considering symmetry and fermionic properties,
which reduces the cognitive load during programming;
optimization of tensor network state programs on supercomputers,
where we performed comprehensive optimization from the ground up,
including adapting low-level matrix operations to supercomputer platforms and adjusting data structures in upper-level algorithm implementations;
software development details for tensor network state algorithms,
culminating in a framework for developing tensor network algorithms,
from an abstract tensor library for various types to a user-friendly interface for various tensor network algorithms;
application of tensor network state algorithms in the Hubbard model,
specifically analyzing the ground state properties on a hexagonal lattice,
where we scanned a broad parameter range of the Hubbard model and found that superconducting properties in this range are suppressed,
while also analyzed potential competing orders such as charge density waves and antiferromagnetism;
and a finite-temperature computational method based on tensor network states,
where we applied our proposed finite-temperature algorithm to various systems including the Hubbard model on square lattices,
showing that our algorithm can yield reliable observables compared to benchmark results.
#+begin_export latex
\end{abstract*}
#+end_export
...
我们从底层算子的优化入手，全面构建了用于张量网络态计算的 TNSP 套件，可以高效实现多种张量网络态算法，特别是针对投影纠缠对态的各类方法。
...
为了应对这些挑战，我们需要采取以下措施：改进算法以提高计算效率和降低复杂度，例如在新型异构计算平台上进行加速；
