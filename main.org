...
上世纪发展的郎道 {{{F}}} 液体理论和基于自发对称性破缺的相变理论在强关联系统中失效。
...
因此，传统 {{{ED}}} 方法（ED）仅适用于约30个格点的有限系统求解。
在某些特殊的强关联系统中，{{{QMC}}} 方法(QMC)能够给出优异结果，但有阻错的系统存在符号问题，为了收敛它依然必须指数增加采样量。
另一大类方法则使用 {{{Ansatz}}} (Ansatz)函数来表示波函数，如何用有限数据量表示庞大的希尔伯特空间中的向量是这类数值方法的出发点。
