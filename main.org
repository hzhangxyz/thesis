...
\end{equation}
为此，张量需要满足形式：
\begin{equation}\label{eq:Abel对成性张量的原始形式}
T_{(s_0,\alpha_{s_0}),(s_1,\alpha_{s_1}),\cdots (s_{R-1},\alpha_{s_{R-1}})} =
...
\end{equation}
是对称群决定的对称性条件。
将公式 [[eqref:eq:Abel对成性张量的原始形式]] 稍作变形得到：
\begin{equation}\label{eq:Abel对成性张量的形式}
T_{(s_0,\alpha_{s_0}),(s_1,\alpha_{s_1}),\cdots (s_{R-1},\alpha_{s_{R-1}})} =
...
TNSP简化了各种张量网络态（TNS）方法，特别是PEPS方法的编程过程，它抽象了不同类型张量的细节，无论是费米子张量、对成性张量还是常规的张量。
...
一个分块多维数组， 或者说Core类型， 是由这些边的序列通过公式  [[eqref:eq:Abel对成性张量的形式]] 创建而成的， 为了性能效率， 将分块数据存储为一维数组。
...
2. \texttt{clear\_symmetry} 函数旨在最大程度地消除对称信息。对于非费米子对成性张量，此操作生成一个非对成性张量。反之，对于费米子张量，该操作的结果是一个费米-$Z(2)$ 对成性张量。
...
+ =utility.py=  包含各种工具函数。
+ =auxiliaries=  目录，利用 =lazy_graph= ，如图 [[ref:fig:张量网络态计算套件的结构]] 所述，用于构型类型，包含几个辅助系统，旨在最小化在边界MPO方法中张量网络收缩的冗余计算，特别是移除或替换多个格点的张量时。
...
def abstract_state(L1, L2):
    # Create an abstract state with size of L1 * L2,
    # and tensor type of fermi-Z(2) symmetry double-precision real number tensor
    state = tet.AbstractState(TAT.FermiZ2.D.Tensor, L1, L2)
    # Set one orbit for each site
    for l1 in range(L1):
        for l2 in range(L2):
            state.physics_edges[l1, l2, 0] = [(parity(+1), 1), (parity(-1), 1)]
    # Retrieve the predefined Hamiltonian term CC
    CC = tet.common_tensor.Parity.CC.to(float)
    # Apply the term to all neighboring site pairs in OBC
    for l1 in range(L1):
        for l2 in range(L2):
            if l1 != 0:
