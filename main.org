...
其中
\begin{equation}
R(s_0, s_1, \cdots, s_{R-1}) = \left\{\begin{aligned}
1 \quad & \text{如果} \quad \prod_r^{R-1} U_g^{s_r} = 1 ,\\
0 \quad & \text{其他情况},
\end{aligned}\right.
\end{equation}
是对称群决定的对称性规则。
...
R(s_0,s_1,\cdots s_{R-1}) .
\end{equation}
可见Abel群的对称性张量是一个分块的张量，其中分块方案由对称性规则和张量每个边上的不可约表示分解决定。

不可约表示 $s$ 中群元素 $s$ 是 $U_g^s$ ，作为一个一维的酉矩阵，它就是一个复数，所以
