...
当空穴掺杂较小时，系统表现出反铁磁序，但在 $\delta > 0.05$ 时该序减弱。
相反地，随着掺杂水平的增加，电荷密度波序逐渐减弱。
...
为验证算法的有效性，我们对二维反铁磁海森堡模型、J1-J2模型和费米哈伯德模型进行了测试。
物理性质如内能、比热和磁化率等的计算结果与随机序列展开（SSE）、严格对角化和行列式量子蒙特卡洛（DQMC）方法的结果进行了对比，
