...
一个分块多维数组， 或者说Core类型， 是由这些边的序列通过公式  [[eqref:eq:Abel对称张量的形式]] 创建而成的， 为了性能效率， 将分块数据存储为一维数组。
...
2. \texttt{clear\_symmetry} 函数旨在最大程度地消除对称信息。对于非费米子对称张量，此操作生成一个非对称张量。反之，对于费米子张量，该操作的结果是一个费米-$Z(2)$ 对称张量。
...
哈密顿量是各项局部哈密顿量的和，即邻近位置 $i$, $j$ 的两体相互作用 $h_{i,j} = c^\dag_i c_j + c^\dag_j c_i$ 。
...
def initialH():
    # Prepare the physical edge and its conjugate
    edge_out = ([(parity(+1), 1), (parity(-1), 1)], False)
    edge_in  = ([(parity(+1), 1), (parity(-1), 1)], True)
    # Set the local Hamiltonian with zero filled first
    H = Tensor(
        ["Oi", "Oj", "Ii", "Ij"],
        [edge_out, edge_out, edge_in, edge_in],
    ).zero_()
    # Set the non zero item of the local Hamiltonian
...
def initialGate(H, tau):
    # Get the quantum gate from the local hamiltonian term
