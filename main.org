...
至此，我们可以得到任意用于表示量子态的函数的参数空间中的度规，这样我们可以利用公式 [[eqref:eq:自然梯度法]] ，
...
\While{$\beta$ 没有降温到目标温度}{
  ...
  $E, G, g \gets \texttt{VMC}(O, |\rho(\beta)\rangle_\sharp)$ \Comment*[r]{使用 \ref{sec::各种采样方法} 中的方法采样得到梯度和度规}
...
在自然梯度法中，我们需要计算度规的逆乘以梯度，这个任务可以通过共轭梯度法实现。
