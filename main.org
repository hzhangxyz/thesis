...
故而我们可以不考虑对称性性质与费米子性质地设计{{{TNS}}}的算法，这减轻了编程工作时的心智负担；
{{{TNS}}} 的程序在超级计算机上的优化工作，我们从下而上全尺度地对现有程序进行了优化，下至底层的矩阵运算对超级计算机平台的适配，上至上层算法实现中数据结构的调整；
用于 {{{TNS}}} 算法的软件开发细节，我们最终发布了一个用于 {{{TNS}}} 算法开发的框架，下至抽象掉不同种类张量细节的张量库，
上至一个为各种 {{{TNS}}} 算法提供了统一用户友好界面的上层接口；
{{{TNS}}} 算法在{{{Hubbard}}}模型中的应用，具体来说，是在 {{{L6}}}上基态的各种性质的分析，
我们对较宽参数范围的 {{{Hubbard}}} 模型进行了扫描，发现范围内的{{{Hubbard}}} 模型基态超导性质并不显著，
同时分析了可能的竞争序，包括{{{CDW}}}以及{{{AFM}}}；
一种基于{{{TNS}}}的有限温系统计算方法，我们将我们提出的有限温算法应用于包括 {{{L4}}}上{{{Hubbard}}}模型的各种系统上，
得到测试结果与基准结果对比显示我们的算法可以得到可靠的观测值。
#+begin_export latex
\end{abstract}
#+end_export
...
展望未来，张量网络态方法，包括专门针对费米子系统的张量网络态方法，仍将持续发展和不断完善。
