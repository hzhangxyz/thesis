...
\end{equation}
其中 $g \in G$ ，
$U_g^{\otimes R} = \otimes_{r=0}^{R-1} U_g^r$ ， $U_g^r$ 为 $g$ 在张量的第 $r$ 个指标的线性空间上的表示。
...
\begin{equation}
V \cong \oplus_{s} d_s V^{s} \cong \oplus_{s} (D^{s} \otimes V^{s}),
\end{equation}
其中 $V^s$ 是群的第 $s$ 个不可约表示空间， $d_s$ 是他的重数，这里 $D^s$ 是 $d_s$ 维的简并空间。

我们可以使用 $U_g^r$ 来作张量的第 $r$ 个指标的基，每个指标的空间 $V^r = \oplus_s (D^{r,s} \otimes V^s)$ 中，使用 $(s,\alpha_s, m_s)$ 来作为基底。
...
+ 2阶 :: 其非零元素的两个指标需要在相同的不可约表示空间中，且两者的 $m_s$ 需要相等。
+ 3阶 :: 张量需要满足形式 $T_{(s_0,\alpha_{s_0},m_{s_0}),(s_1,\alpha_{s_1},m_{s_1}),(s_2),\alpha_{s_2},m_{s_2}} = P_{(s_0, \alpha_{s_0}),(s_1, \alpha_{s_1}),(s_2, \alpha_{s_2})} \langle s_0 m_0 s_1 m_1 | s_2 m_2\rangle$ ，其中 $\langle s_0 m_0 s_1 m_1 | s_2 m_2 \rangle$ 是这个群的克莱布希-高登系数。
+ 更高阶 :: 张量可需要以被分解为多个3阶对称性张量收缩的形式。
