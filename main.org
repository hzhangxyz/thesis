...
图 [[ref:fig:超导power-law]] 展示了在不同掺杂水平下， $U=8$ 时的Cooper对关联函数。
...
这些结果表明，在这一掺杂水平下，六角格子上的Hubbard模型的超导性被抑制了。
...
\note{注：
在198个格点的六角格子上，对于不同的空穴掺杂水平 $\delta$ ，计算了 $U = 8$ 时的单态和三态Cooper对关联函数 $C(\Delta r)$ 。
...
随着掺杂水平的增加，衰减指数 $K_s$ 和 $K_t$ 的值趋于减小。
对于 $U = 4$ 且掺杂水平 $\delta$ 超过 $0.05$ 的情况， 参数 $K$ 始终小于2。
...
\caption{The power law decay exponent (a) $K_s$ of the singlet pair correlation
  functions and (b) $K_t$ of the triplet pair correlation
functions as a function of hole doping $\delta$, for different interaction strengths $U$, on a 198-site honeycomb lattice.
}
\label{fig:k-plot}
\end{figure}
#+end_export
