...
- SR :: 随机重构型（Stochastic Reconfiguration）
- SSE :: 随机交换方法（Stochastic Series Expansion）
- DQMC :: 行列式量子蒙特卡洛（Determinant Quantum Monte Carlo）
...
+ 第 [[ref:sec:超导]] 章介绍在六角晶格中的Hubbard模型基态中寻找超导相的工作。
...
当 $J_{1} = 1$ 且 $J_{2} = 0$ 时，公式 [[eqref:eq:j1-j2]] 即退化为无阻挫的海森堡模型，因此可以通过量子蒙特卡罗（QMC）方法进行模拟而不会遇到符号问题，例如基于随机交换方法（SSE） [[cite:Sandvik1991,Sandvik1992]] 。
如图 [[ref:fig:thermal-heisenberg]] 所示，我们将得到的结果与SSE方法进行比较。
具体的模型和计算参数为，大小为 $N = 8 \times 8$ 的方形晶格，采用开放边界条件。
我们设定投影纠缠对态中的虚拟指标维度为 $D=8$ 。
我们使用边界MPO收缩算法 [[cite:Verstraete2008,Liu2021]] 来收缩单层张量网络， 其精度由截断维度 $D_c$ 控制，这里我们设定 $D_c =16$ 。
