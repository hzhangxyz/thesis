...
\begin{abstract}
#+end_export
#+LATEX: {\color{blue}ZH: 董少钧建议标题使用 ``张量网络方法的发展及其在费米子哈伯德模型中的应用''，可能这样更连贯，可以么？}

上世纪发展起来的朗道费米液体理论和基于自发对称性破缺的相变理论在研究许多凝聚态问题上取得了巨大的成功。
然而，这些理论在强关联系统中失效，因为在这类系统中，粒子间存在较强的关联相互作用，导致传统的微扰理论无法得到准确的结果。
...
特别是，由于缺乏专门支持费米子张量网络态的基础软件包，这进一步限制了 费米子 {{{TNS}}} 在实际计算中的推广和应用。
...
1. TAT（底层张量库）：支持不同类型张量的复杂运算，包括收缩以及各种各种分解，并提供了统一的接口掩盖对称性和费米子性质以便上层使用。
...
我们进一步分析了超导性与其他竞争序，包括反铁磁 (AFM) 序和电荷密度波 (CDW) 序，之间的关系。
#+LATEX: {\color{blue}结果显示，系统会表现出反铁磁序和电荷密度波序。}
...
#+LATEX: {\color{blue}ZH: 原来的 ``当空穴掺杂较小时，系统表现出反铁磁序和电荷密度波序'' 和后文联系起来可能有歧义。这里的小dop是只我们的参数范围本身小，后文的小指范围内 $\delta<0.05$ 的小，故修改为"结果显示，系统会表现出反铁磁序和电荷密度波序"。}
...
1. To simplify the programming implementation of symmetry tensor network states and fermionic tensor network states, we propose and implement a new scheme.
   ...
   This improvement allows ignoring the complex details of symmetry tensor networks and fermionic tensor networks in algorithm implementation, simplifying the handling of symmetry and fermionic properties,
   ...
   (1) TAT (Basic tensor library): Supports complex operations for different types of tensors, including contractions and various decompositions,
   and provides a unified interface to abstract away symmetry and fermion properties for use by higher-level applications.
...
2. We utilized the fermionic projected entangled pair states (f-PEPS) method to study the Hubbard model with hole doping on a hexagonal lattice.
   ...
   The results indicate that the system exhibits both antiferromagnetic and charge density wave orders.
   Notably, when the hole doping $\delta$ is small and the Coulomb parameter $U$ is large, antiferromagnetic domain wall structures exist within the system.
   As the hole doping $\delta$ increases or $U$ decreases, these domain wall structures disappear.
   When $\delta > 0.05$ , the antiferromagnetic order weakens and eventually vanishes.
   With increasing doping levels, the charge density wave order also gradually diminishes, but it is noteworthy that a significant charge density wave order can still be observed even at higher doping levels.
...
首先求解无相互作用的自由系统 $H_f$，这时的多体系统可以简单地表示为子空间解的张量积，随后再引入相互作用 $H_i$ 作为微扰，获得含有相互作用的系统的结果。
...
#+LATEX: {\color{blue}ZH: 这段的思路： 首先将物理产生算符变成fermion \to 发现纠缠对不boson了 \to 将纠缠对另一半也换成fermion的 \to 整体又是boson的了 \to 类似的虚拟纠缠对也可以替换成fermion的 \to 投影算子不boosn了 \to 下文的对称性将解决这个问题。 }
...
基于包括 {{{MPS}}} 和 {{{PEPS}}} 在内的 各种 {{{TNS}}} 作为一种高效表示量子态的方法，受到了广泛关注。
...
而针对一维系统，逐块虚时间演化方法结合了 {{{MPS}}} 和 虚时间演化方法的优势，可以以极高的精度得到系统的基态。
而对于二维系统， {{{PEPS}}} 难以严格地进行虚时间演化，故 简单更新方法 [[cite:Jiang2008]] 被提出以便以极低的代价获得系统的低能态。

{{{MC}}} 方法作为一个古老的方法，在 {{{TNS}}} 上也有广泛应用， 通过 {{{VMC}}} 方法 [[cite:Sandvik2007,Liu2017]] ，
...
SVD后舍弃小的奇异值，是低秩近似的实现 [[cite:Schmidt1907,Eckart1036,Mirsky1960]]。
...
{{{TNSP}}} 实现为 C++17 ，并未没有使用针对平台的扩展，使用神威上的编译器编译后， 其可以成功地在主核上运行。
...
我们还在国产超级计算机神威 \cdot 太湖之光上， 成功移植了 {{{TNSP}}} 软件。
在本移植工作中，也发现过神威 \cdot 太湖之光系统中的一些问题，通过反馈，参与了国产超级计算机的生态建设。
