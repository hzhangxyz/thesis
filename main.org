...
在实际应用中，尤其对仅具有近邻和次近邻相互作用的格点模型，我们通常将哈密顿量拆分成尽可能少的不对易部分，以最大程度地减少误差。
例如，对于一维仅存在近邻相互作用的系统，哈密顿量可以表示为
\begin{equation}
H = \sum_{i=0,\cdots,L-2} h_{i,i+1} ,
\end{equation}
其中 $h_{i,i+1}$ 表示第 $i$ 和 $i+1$ 个格点之间的相互作用。
我们可以将哈密顿量分为两部分 $H=H_0 + H_1$ ，其中
\begin{equation}
\left\{
\begin{aligned}
H_0 &= \sum_{i=0,2,\cdots} h_{i,i+1} \\
H_1 &= \sum_{i=1,3,\cdots} h_{i,i+1} .
\end{aligned}
\right.
\end{equation}
...
* 六角格子中的Hubbard模型的掺杂基态模拟计算

TODO

* 方格子中的Hubbard模型的有限温模拟计算
